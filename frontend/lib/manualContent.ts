...
export const MANUAL_SECOES: ManualSectionData[] = [
  ...
  },
  {
    ...
    itens: [
      ...
      { t: 'Desconto — % ou R$', d: 'No fechamento, escolha percentual ou valor fixo e digite livremente o desconto. O total é recalculado na hora e o desconto aparece separado no histórico, nos relatórios e no documento fiscal.' },
      { t: 'Comentário do cliente', d: 'O cliente pode enviar na própria comanda observações como ponto da carne, retirada de ingrediente ou alergia. O comentário aparece em tempo real na comanda do atendimento e no painel Restaurante.' },
      { t: 'Produção por área', d: 'No módulo Restaurante, vincule cada produto à Cozinha, Bar ou outra área. Quando entra na comanda, o item aparece automaticamente na fila e avança por Recebido, Preparando, Pronto e Servido.' },
    ...
    dicas: [
      ...
      'Comandas, mesas, QR Codes e fila de produção pertencem ao módulo Restaurante. Ao desativá-lo, a operação some dos menus e APIs, mas o histórico financeiro continua preservado.',
    ],
  },
  {
    num: '03',
    ...
    itens: [
      ...
      { t: 'Desconto — % ou R$', d: 'Na etapa de pagamento, escolha percentual ou valor fixo em reais e digite qualquer valor. Os atalhos de 0%, 5%, 10%, 15% e 20% continuam disponíveis; o desconto padrão é pré-configurável nas Preferências.' },
  ...
  },
  {
    ...
    itens: [
      ...
      { t: 'Insights Financeiros', d: 'Abra Financeiro → Insights para reunir os sinais da DRE, capital de giro e estoque em uma fila de decisões. Itens urgentes aparecem primeiro e, quando existe um valor conhecido, os maiores impactos ganham prioridade.' },
      { t: 'Por que o insight apareceu', d: 'Use o botão ? de cada recomendação para consultar a regra que disparou o sinal. Os insights não usam uma nota oculta e nenhuma ação altera preços, estoque, contas ou cobranças automaticamente.' },
      { t: 'Comparação equivalente', d: 'Receita e resultado são comparados com o intervalo imediatamente anterior usando a mesma quantidade de dias. Quando a base anterior é zero, o sistema não apresenta uma variação percentual artificial.' },
      { t: 'Qualidade da leitura', d: 'A central avisa quando faltam custos, classificações da DRE ou compras para estimar o prazo de pagamento. Corrigir esses pontos melhora todos os indicadores relacionados.' },
      ...
      { t: 'Preço e Rentabilidade', d: 'Abra Financeiro → Preço e Rentabilidade para comparar margem bruta e markup por produto, definir uma meta e simular o preço necessário. A simulação não altera o cadastro do produto.' },
      { t: 'Margem x markup', d: 'Margem bruta usa a receita como base: (venda − custo) ÷ venda. Markup usa o custo como base: (venda − custo) ÷ custo. Por isso os percentuais são diferentes mesmo para o mesmo produto.' },
      { t: 'Preço sugerido', d: 'É calculado por custo médio ÷ (1 − margem desejada). Nesta primeira etapa ele considera somente o custo do produto; impostos, taxas de cartão, comissões e frete serão tratados na margem de contribuição.' },
      { t: 'Margem de contribuição', d: 'Abra Financeiro → Ponto de Equilíbrio para calcular quanto sobra da receita depois de CMV, impostos sobre vendas, taxas de cartão, comissões, frete e outros custos variáveis. Essa sobra paga as despesas fixas e depois forma lucro.' },
      { t: 'Premissas editáveis', d: 'Receita, CMV e impostos vêm da DRE. Como o cadastro ainda não separa despesas fixas de variáveis, a tela carrega despesas operacionais como base fixa. Taxas de cartão, comissões e frete podem ser salvos como padrões da loja; a despesa fixa continua específica de cada simulação.' },
      { t: 'Ponto de equilíbrio', d: 'Divide as despesas fixas pela margem de contribuição percentual. O resultado mostra a receita necessária para cobrir custos variáveis e estrutura sem lucro nem prejuízo.' },
      { t: 'Margem de segurança', d: 'É a diferença entre a receita simulada e o ponto de equilíbrio. Valor negativo indica que o cenário não cobre a estrutura informada.' },
      { t: 'Simulador de desconto', d: 'Mantém quantidade e CMV do período, reduz a receita e recalcula os custos variáveis percentuais. A tela mostra o limite para continuar contribuindo e o limite mais conservador para ainda cobrir as despesas fixas.' },
      { t: 'Custo ausente', d: 'Quando uma venda não possui custo congelado, o sistema sinaliza o produto e não calcula margem, markup nem preço sugerido, evitando apresentar um lucro artificial.' },
      { t: 'Capital de Giro', d: 'Abra Financeiro → Capital de Giro para acompanhar estoque imobilizado, contas a receber, contas a pagar e a necessidade de recursos da operação. A fórmula é estoque + receber − pagar.' },
      { t: 'Ciclo financeiro estimado', d: 'Soma os dias de cobertura do estoque ao prazo de recebimento e subtrai o prazo de pagamento. Como o sistema ainda não possui snapshots diários de estoque, usa o saldo atual e identifica claramente o resultado como estimativa.' },
      { t: 'Pressão de curto prazo', d: 'Destaca recebimentos vencidos, pagamentos vencidos e contas que vencem nos próximos 7 dias, além de apontar atalhos para cobrança, contas e estoque.' },
      { t: 'Projeção de Caixa', d: 'Abra Financeiro → Projeção de Caixa para organizar crediários e outros títulos abertos por vencimento. Escolha entre 7, 15, 30 ou 60 dias e informe o saldo disponível para antecipar pressão no caixa.' },
      { t: 'Cenário conservador', d: 'Por padrão, a projeção usa somente recebimentos e pagamentos já cadastrados. Vendas futuras não entram automaticamente; informe uma geração líquida diária apenas quando desejar simular essa expectativa.' },
      { t: 'Reserva mínima', d: 'Define o caixa que você deseja preservar. A tela encontra o menor saldo, o primeiro dia abaixo da reserva e o valor necessário para evitar esse déficit.' },
      { t: 'Padrões de caixa', d: 'A geração líquida diária e a reserva mínima podem ser salvas para os próximos acessos. O saldo disponível não é salvo porque muda diariamente e deve refletir o caixa real no momento da análise.' },
      { t: 'Títulos vencidos e sem data', d: 'Vencidos entram no primeiro dia da agenda por prudência. Títulos sem vencimento ficam fora da linha do tempo e aparecem em um alerta para correção, evitando inventar uma data.' },
      { t: 'Estoque Inteligente', d: 'Abra Financeiro → Estoque Inteligente para cruzar o saldo atual com as vendas do período. A tela classifica risco de ruptura, cobertura baixa, excesso, ausência de movimento e produtos sem custo.' },
      { t: 'Cobertura e reposição', d: 'Cobertura é estoque atual ÷ venda média diária. Ajuste a meta em dias para simular uma quantidade de reposição; a sugestão não altera o cadastro nem cria uma compra automaticamente.' },
      { t: 'GMROI estimado', d: 'Divide a margem bruta gerada no período pelo valor do estoque atual. É identificado como estimado porque ainda não existe histórico de estoque médio diário.' },
    ...
    dicas: [
      ...
      'Use os botões ? na página de rentabilidade para consultar a fórmula e a interpretação de cada indicador sem sair da análise.',
      'Comece pelos itens Urgentes em Financeiro → Insights; depois trate os pontos de Atenção e avalie as Oportunidades.',
    ],
  },
  {
    num: '14',
  ...
  },
  {
    ...
    itens: [
      ...
      { t: 'CRM de leads', d: 'Painel com captação da landing e prospecção, busca, etapas Novo/Contatado/Convertido/Perdido, indicadores de conversão, score, anotações e conversão direta em loja cliente.' },
