...
function InvitationForm() {
  ...
  useEffect(() => {
    ...
    }).catch(error => toast.error(getErrorMessage(error, 'Convite inválido ou expirado.'))).finally(() => setLoading(false))
  }, [token])

  async function accept(event: React.FormEvent) {
    event.preventDefault(); setSaving(true)
    try { await publicReferralApi.accept(token, form); setDone(true) }
    catch (error) { toast.error(getErrorMessage(error, 'Não foi possível concluir o cadastro.')) }
    finally { setSaving(false) }
  }

  if (loading) return <main className="grid min-h-screen place-items-center bg-slate-950"><Spinner /></main>
  if (done) return <main className="grid min-h-screen place-items-center bg-slate-950 p-4"><div className="max-w-lg rounded-2xl border border-emerald-500/30 bg-slate-900 p-8 text-center text-white"><CheckCircle2 className="mx-auto mb-4 h-12 w-12 text-emerald-400" /><h1 className="text-2xl font-bold">Parceria registrada</h1><p className="mt-2 text-slate-400">Seus dados e o aceite foram registrados. A equipe 3ESYSTEN dará continuidade ao processo.</p></div></main>
  if (!invitation) return <main className="grid min-h-screen place-items-center bg-slate-950 p-4 text-center text-slate-300"><div><h1 className="text-xl font-bold text-white">Convite indisponível</h1><p className="mt-2">O link pode estar inválido, expirado, revogado ou já ter sido utilizado.</p></div></main>

  return <main className="min-h-screen bg-slate-950 px-4 py-10 text-slate-200"><form onSubmit={accept} className="mx-auto max-w-3xl space-y-6 rounded-2xl border border-slate-800 bg-slate-900 p-5 sm:p-8">
    ...
    <Button type="submit" loading={saving} disabled={!form.acceptedTerms}>Aceitar e concluir cadastro</Button>
  </form></main>
}
