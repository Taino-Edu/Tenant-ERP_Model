        ...
        <div className="space-y-4">
          <form onSubmit={saveGovernance} className="space-y-3 rounded-xl border border-surface-600 p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label><span className="label">Parceiro de indicação</span><select className="input w-full" value={referralPartnerId} onChange={event => setReferralPartnerId(event.target.value)}><option value="">Sem indicação</option>{partners.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
