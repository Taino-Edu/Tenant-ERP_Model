...
public class TenantCharge
{
    ...
    // -------------------------------------------------------------------------
    // Origem nas condições negociadas (ver CondicoesComerciais)
    // -------------------------------------------------------------------------

    /// <summary>True quando a linha nasceu do gerador a partir das condições da
    /// loja. Só estas acompanham uma renegociação enquanto estão em aberto.
    /// Lançamento manual — ou cobrança automática editada à mão depois — fica
    /// como está: a edição manual é uma decisão explícita, e o recálculo passar
    /// por cima dela desfaria o ajuste sem ninguém perceber.</summary>
    [Column("auto_generated")]
    public bool AutoGenerated { get; set; }

    /// <summary>Valor antes dos descontos. Null em lançamento manual e no
    /// histórico anterior às condições negociadas.</summary>
    [Precision(10, 2)]
    [Column("gross_amount")]
    public decimal? GrossAmount { get; set; }

    /// <summary>Quanto os descontos tiraram de <see cref="GrossAmount"/>.</summary>
    [Precision(10, 2)]
    [Column("discount_amount")]
    public decimal DiscountAmount { get; set; }

    /// <summary>Quais descontos entraram, como foto do momento ("Parceiro 10% +
    /// Boas-vindas R$ 30,00"). Foto e não referência pelo mesmo motivo de
    /// <see cref="Amount"/>: apagar o desconto amanhã não pode tirar do
    /// histórico a explicação de por que março saiu mais barato.</summary>
    [MaxLength(300)]
    [Column("discount_summary")]
    public string? DiscountSummary { get; set; }

    /// <summary>Número da parcela da implantação (1, 2, 3...). Null em
    /// mensalidade e em implantação lançada de uma vez à mão.</summary>
    [Column("installment_number")]
    public int? InstallmentNumber { get; set; }

    /// <summary>Total de parcelas quando esta foi gerada ou recalculada.</summary>
